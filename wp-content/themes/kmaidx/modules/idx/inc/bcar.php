...
class bcar_idx {
	...
	public function __construct() {
        ...
        wp_enqueue_script( 'bcar-scripts' );
...
add_action('wp_ajax_loadBcarIdx', 'loadBcarIdx');
add_action('wp_ajax_nopriv_loadBcarIdx', 'loadBcarIdx');
function loadBcarIdx() {
...
wp_register_script( "bcar-scripts", get_template_directory_uri() . '/modules/idx/scripts/bcar.ajax.js', array('jquery'), '0.0.0' , true );
wp_localize_script( 'bcar-scripts', 'wpAjax', array( 'ajaxurl' => admin_url( 'admin-ajax.php' )));

add_action('wp_ajax_removeVar', 'removeVar');
add_action('wp_ajax_nopriv_removeVar', 'removeVar');
function removeBcarVar() {

	if ( $_POST['remove'] != '' ) {
		$_SESSION[ $_GET['remove'] ] = '';
		//$currentUrl                  = $_SERVER['SELF'];
		$result                      = json_encode( $_POST['remove'] );
	}
